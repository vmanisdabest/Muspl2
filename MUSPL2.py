...
def DisToScr(XY):
    ...
    EXY = list(XY)
    if DSTF:
        XSCL = round(display_screen.get_width()/900, 5)
        YSCL = round(display_screen.get_height()/700, 5)
        EXY[0] = round(EXY[0]/XSCL)
        EXY[1] = round(EXY[1]/YSCL)
    else:
                   ...
                   if keys[pygame.K_UP]:
                       ...
                       if PPOS == 2:
                           ...
                           while not keys[pygame.K_ESCAPE]:
                               ...
                               CHK_PLAYER()
                               if DSTF:
                                   display_screen.blit(pygame.transform.scale(screen.copy(), [display_screen.get_width(), display_screen.get_height()]), [0,0])
                               elif display_screen.get_width() != 900 or display_screen.get_height() != 700:
                   ...
                   if pygame.mouse.get_pressed()[0]:
                       ...
                       if XY[1] >= 0 and XY[1] <= 60:
                           if XY[0] <= 143:
                               ...
                               SLC = 4
                       elif XY[1] >= 585 and XY[1] <= 604 and SLC == 3:
                            if XY[0] <= 329:
                                POS = 1
                            elif XY[0] <= 447:
                                POS = 2
...
if not os.path.isfile("Readme.txt") and not os.path.isfile("Muspl2-Settings.txt"):
    ...
    FILE.write("NXT = True\n")
    FILE.write("INIT_VOL = 50\n")
    FILE.write("DSTF = False\n")
    FILE.close()
